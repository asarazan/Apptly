...
public class AppApiEngine {
  ...
  static final String rawApiGet(URL target, Credentials credentials) throws IOException {
    HttpURLConnection conn = (HttpURLConnection) target.openConnection();
    try {
      conn.setRequestProperty("Authorization", "Bearer " + credentials.getAuthToken());
  ...
  public static final Profile getSelfProfile(Credentials credentials) {
    URL target = null;
    ...
    try {
      String r = rawApiGet(target, credentials);
